...
const destinationCountriesList = {
  "Sweden (7.35 INR)": 7.35,
  "China (11.53 INR)": 11.53,
  "Brazil (15.63 INR)": 15.63,
  "Australia (50.09 INR)": 50.09,
};

const AddBoxForm = () => {
  const [receiverName, setReceiverName] = useState("");
  const [boxWeight, setBoxWeight] = useState("");
  const [boxColor, setBoxColor] = useState("rgb(255,255,255)");
  const [destinationCountry, setDestinationCountry] = useState("");
  const [error, setError] = useState(null);
  const dispatch = useDispatch();

  const inputConatiners = "flex flex-col space-y-1";
  const inputWrapper = "border border-gray-300 rounded p-1 w-[50%]";
  ...
  return (
    <div className="max-w-xl mx-auto">
      <h2 className="text-2xl font-semibold mb-4">Add Box Form</h2>
